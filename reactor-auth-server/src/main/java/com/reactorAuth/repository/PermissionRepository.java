...
public interface PermissionRepository extends R2dbcRepository<Permission, Long>, PageableRepository<Permission> {
    ...
    @Query("""
        SELECT p.method, p.url, r.code FROM permissions p
        INNER JOIN role_permissions rp ON rp.permission_id = p.id
        INNER JOIN roles r ON r.id = rp.role_id
        WHERE p.status = 1 AND p.url IS NOT NULL
        UNION
        SELECT p.method, p.url, p.code FROM permissions p
        WHERE p.status = 1 AND p.url IS NOT NULL
    """)
    ...
    @Query("SELECT * FROM permissions ORDER BY id DESC LIMIT :limit OFFSET :offset")
